...
const posts: PostData[] = [
    ...
    const normalizeData = (unnormalizedData: PostData[]): NormalizedDataType => {
        let normalized: NormalizedDataType = { byid: {}, allIsd: [] }
        let arrayIds: string[] = []
        for (let i: number = 0; i < unnormalizedData.length; i++) {
            normalized.byid[unnormalizedData[i].id] = unnormalizedData[i]
            arrayIds.push(unnormalizedData[i].id)
        }
        normalized.allIsd = arrayIds
        return normalized as NormalizedDataType
    }
